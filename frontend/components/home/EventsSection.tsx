...
const EventsSection: React.FC = () => {
  ...
  const fetchEvents = async () => {
    try {
      ...
      const publishedEvents = allEvents.filter((e: Event) => e.status === EventStatus.PUBLISHED);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-gray-500 italic">Recherche des meilleures opportunités...</p>
          </div>
        ) : error ? (
          ...
          <div className="text-center py-20 bg-white rounded border border-gray-100">
            <p className="text-gray-400 italic font-light tracking-wide">Plus d'événements prévus pour le moment. Revenez bientôt !</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {events.map((event) => (
              <div key={event._id} className="group bg-white rounded-md border border-gray-100 overflow-hidden hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 flex flex-col h-full">
                ...
                <div className="p-8 flex flex-col flex-grow">
                  ...
                  <Link href={`/events/${event._id}`} className="block mb-4">
                    <h3 className="text-xl font-bold text-gray-900 group-hover:text-[#C5A059] transition-colors leading-tight cursor-pointer" style={{ fontFamily: 'serif' }}>
                      {event.title}
                    </h3>
                  </Link>
                  <div className="flex items-center justify-between mt-auto pt-6 border-t border-gray-100 min-h-[4rem]">
                    <span className="text-[10px] text-gray-400 font-bold flex items-start gap-1 uppercase tracking-wider max-w-[65%]">
                      <MapPin size={14} className="text-[#C5A059] flex-shrink-0 mt-[1px]" />
                      <span className="line-clamp-2 leading-tight">{event.location}</span>
                    </span>
                    <Link href={`/events/${event._id}`} className="text-[#1A1A1A] font-bold text-[10px] hover:text-[#C5A059] transition-colors flex items-center gap-1 uppercase tracking-[0.2em] group/btn flex-shrink-0">
                      Réserver <ChevronRight size={14} className="group-hover/btn:translate-x-1 transition-transform" />
                    </Link>
                  </div>
                </div>
              </div>
